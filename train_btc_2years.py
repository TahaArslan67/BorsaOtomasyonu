...
def train_btc_2years():
    """BTC 2 yıl veri ile eğit"""
    symbol = "BTCUSDT"
    
    client = CryptoExchangeClient()
    
    logger.info(f"BTC 2 yıl veri ile eğit: {symbol}")
    
    # Veri çek (2 yıl = 17520 bar)
    ...
    # Modelleri kaydet
    import joblib
    joblib.dump(model_4h, f"price_prediction_BTC_4h_2years.pkl")
    joblib.dump(model_1d, f"price_prediction_BTC_1d_2years.pkl")
    
    logger.info("=" * 60)
    logger.info("BTC FİYAT TAHMİN MODELLERİ KAYDEDİLDİ (2 YIL VERİ)")
    logger.info("=" * 60)
    logger.info(f"4h Model: price_prediction_BTC_4h_2years.pkl")
    logger.info(f"1gün Model: price_prediction_BTC_1d_2years.pkl")
    logger.info("=" * 60)
