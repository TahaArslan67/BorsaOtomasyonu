"""
GMSTR Saatlik Model - Odaklanmış Yaklaşım
- Son 60 günlük veri (daha güncel pattern)
- Borsa saatleri filtresi
- Daha az ama daha kaliteli özellik
- Hedef: %58+ doğruluk
"""
import sys
import json
...
def load_hourly_data(period="60d"):
    """Yahoo Finance'den saatlik GMSTR verisi çek."""
    try:
        import yfinance as yf
        print(f"  Yahoo Finance'den saatlik veri çekiliyor ({period})...")
        ...
        df = df.sort_index()
        # Sadece borsa saatlerini filtrele (09:00-18:00, Pzt-Cum)
        df = df[(df.index.hour >= 9) & (df.index.hour <= 18)]
        df = df[df.index.dayofweek < 5]
        print(f"  {len(df)} saatlik bar | {df.index[0].date()} -> {df.index[-1].date()}")
        return df
...
def engineer_features(df, resample_hours=1):
    """Odaklanmış özellik mühendisliği."""
    if resample_hours > 1:
        df = df.resample(f'{resample_hours}h').agg({
            'Open': 'first', 'High': 'max', 'Low': 'min',
            'Close': 'last', 'Volume': 'sum'
        }).dropna()
        # Resample sonrası borsa saati filtresi
        df = df[(df.index.hour >= 9) & (df.index.hour <= 18)]
        df = df[df.index.dayofweek < 5]
    ...
    # === TEMEL GETİRİ ===
    for lag in [1, 2, 3, 4, 5, 6, 8, 12, 16, 24]:
        df[f'ret_{lag}'] = close.pct_change(lag)

    # === MOMENTUM ===
    for p in [2, 3, 4, 6, 8, 12, 16, 24, 48]:
        df[f'mom_{p}'] = close / close.shift(p) - 1

    # === VOLATİLİTE ===
    for w in [3, 6, 12, 24]:
        df[f'vol_{w}'] = ret.rolling(w).std()
    df['vol_ratio'] = df['vol_3'] / (df['vol_24'] + 1e-8)
    ...
    df['obv_trend'] = np.sign(obv - obv_ma)
...
def main():
    print("=" * 60)
    print("  GMSTR SAATLİK MODEL - ODAKLANMIŞ EĞİTİM")
    print("=" * 60)

    # Son 60 günlük veri
    df_raw = load_hourly_data(period="60d")
    ...
    for key, r in training_results.items():
        status = "✅" if r['test_accuracy'] >= 0.58 else ("⚠️" if r['test_accuracy'] >= 0.52 else "❌")
