"""
BTC Doğru Backtest (Eğitim Dışı Veriler)
"""
...
def backtest_btc_correct():
    """BTC doğru backtest (eğitim dışı veriler)"""
    symbol = "BTCUSDT"
    
    client = CryptoExchangeClient()
    
    logger.info(f"BTC doğru backtest (eğitim dışı veriler): {symbol}")
    
    # Veri çek (son 1 ay - eğitim dışı)
    ...
    logger.info(f"Veri çekildi: {len(df)} bar (eğitim dışı)")
    ...
    # 2 yıl veri ile eğitilmiş modelleri yükle
    import joblib
    model_price_4h = joblib.load(f"price_prediction_BTC_4h_2years.pkl")
    model_price_1d = joblib.load(f"price_prediction_BTC_1d_2years.pkl")
    ...
    logger.info("=" * 60)
    logger.info("BTC DOĞRU BACKTEST (EĞİTİM DIŞI VERİLER)")
    logger.info("=" * 60)
    ...
    for i in range(len(df) - 4):
        ...
        if i + 4 < len(df):
            ...
            actual_price_4h = df['close'].iloc[i+4]
    ...
    for i in range(len(df) - 24):
        ...
        if i + 24 < len(df):
            ...
            actual_price_1d = df['close'].iloc[i+24]
    ...
    logger.info("=" * 60)
    logger.info("4h BACKTEST SONUÇLARI (EĞİTİM DIŞI)")
    logger.info("=" * 60)
    ...
    logger.info("=" * 60)
    logger.info("1gün BACKTEST SONUÇLARI (EĞİTİM DIŞI)")
    logger.info("=" * 60)
