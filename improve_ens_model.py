...
def improve_ens_model():
    """ENS model geliştirme (daha iyi)"""
    symbol = "ENSUSDT"
    
    client = CryptoExchangeClient()
    
    logger.info(f"ENS model geliştirme (daha iyi): {symbol}")
    ...
    # 4h için model eğit
    logger.info("4h fiyat tahmin modeli eğitiliyor (daha iyi)...")
    X_4h = df[features]
    y_4h = df['target_4h']
    ...
    logger.info("1gün fiyat tahmin modeli eğitiliyor (daha iyi)...")
    X_1d = df[features]
    y_1d = df['target_1d']
    ...
    # Modelleri kaydet
    import joblib
    joblib.dump(model_4h, f"price_prediction_ENS_4h_v2.pkl")
    joblib.dump(model_1d, f"price_prediction_ENS_1d_v2.pkl")
    
    logger.info("=" * 60)
    logger.info("ENS DAHA İYİ FİYAT TAHMİN MODELLERİ KAYDEDİLDİ")
    logger.info("=" * 60)
    logger.info(f"4h Model: price_prediction_ENS_4h_v2.pkl")
    logger.info(f"1gün Model: price_prediction_ENS_1d_v2.pkl")
    logger.info("=" * 60)
