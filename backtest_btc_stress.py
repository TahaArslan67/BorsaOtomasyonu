"""
BTC Geliştirilmiş Model Stres Test
"""
...
def backtest_btc_stress():
    """BTC geliştirilmiş model stres test"""
    symbol = "BTCUSDT"
    
    client = CryptoExchangeClient()
    
    logger.info(f"BTC geliştirilmiş model stres test: {symbol}")
    ...
    # Volatil dönemler seç (ATR yüksek olan dönemler)
    df['atr_percent'] = df['atr'] / df['close'] * 100
    high_volatility = df[df['atr_percent'] > df['atr_percent'].quantile(0.75)]
    
    logger.info(f"Volatil dönemler: {len(high_volatility)} bar (top %{len(high_volatility)/len(df)*100:.1f})")
    ...
    logger.info("=" * 60)
    logger.info("BTC GELİŞTİRİLMİŞ MODEL STRES TEST (VOLATİL DÖNEMLER)")
    logger.info("=" * 60)
    
    # Geliştirilmiş modelleri yükle
    import joblib
    model_4h = joblib.load(f"price_prediction_BTC_4h_improved.pkl")
    model_1d = joblib.load(f"price_prediction_BTC_1d_improved.pkl")
    
    # 4h backtest (volatil dönemler)
    for i in range(len(high_volatility) - 4):
        X = high_volatility[features].iloc[i:i+1]
        current_price = high_volatility['close'].iloc[i]
        
        if i + 4 < len(high_volatility):
            pred_price_4h = model_4h.predict(X)[0]
            actual_price_4h = high_volatility['close'].iloc[i+4]
    ...
    # 1gün backtest (volatil dönemler)
    balance_1d = initial_balance
    
    for i in range(len(high_volatility) - 24):
        X = high_volatility[features].iloc[i:i+1]
        current_price = high_volatility['close'].iloc[i]
        
        if i + 24 < len(high_volatility):
            pred_price_1d = model_1d.predict(X)[0]
            actual_price_1d = high_volatility['close'].iloc[i+24]
    ...
    logger.info("=" * 60)
    logger.info("4h STRES TEST SONUÇLARI (VOLATİL DÖNEMLER)")
    logger.info("=" * 60)
    ...
    logger.info("=" * 60)
    logger.info("1gün STRES TEST SONUÇLARI (VOLATİL DÖNEMLER)")
    logger.info("=" * 60)
    ...
    if profit_pct_4h > 0:
        logger.info("4h modeli volatil dönemlerde kârlı ✓")
    else:
        logger.info("4h modeli volatil dönemlerde zararlı ✗")
    
    if profit_pct_1d > 0:
        logger.info("1gün modeli volatil dönemlerde kârlı ✓")
    else:
        logger.info("1gün modeli volatil dönemlerde zararlı ✗")
