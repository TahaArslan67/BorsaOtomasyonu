"""
BTC Canlı Yön Tahmin Botu
%70.14 (4h) ve %89.58 (1gün) doğrulukta
"""
...
def live_btc_bot():
    ...
    # Yön tahmin modellerini yükle (%70.14 4h, %89.58 1gün)
    import joblib
    ...
    model_price_4h = joblib.load(f"price_prediction_BTC_4h.pkl")
    model_price_1d = joblib.load(f"price_prediction_BTC_1d.pkl")
    ...
    logger.info("=" * 60)
    logger.info("BTC CANLI YÖN TAHMİN BOTU BAŞLADI")
    logger.info("=" * 60)
    logger.info("4h Doğruluk: %70.14")
    logger.info("1gün Doğruluk: %89.58")
    logger.info("=" * 60)
        ...
        try:
            ...
            try:
                ...
                if signal_4h_changed or signal_1d_changed or price_changed:
                    ...
                    if signal_4h == 1:
                        logger.info(f"4h: YÜKSELİŞ bekleniyor (Doğruluk: %70.14) | Tahmin: ${pred_price_4h:.2f}")
                    else:
                        logger.info(f"4h: DÜŞÜŞ bekleniyor (Doğruluk: %70.14) | Tahmin: ${pred_price_4h:.2f}")
                    
                    if signal_1d == 1:
                        logger.info(f"1gün: YÜKSELİŞ bekleniyor (Doğruluk: %89.58) | Tahmin: ${pred_price_1d:.2f}")
                    else:
                        logger.info(f"1gün: DÜŞÜŞ bekleniyor (Doğruluk: %89.58) | Tahmin: ${pred_price_1d:.2f}")
