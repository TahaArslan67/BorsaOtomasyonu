"""
GMSTR Model Güncelleme (Son 1 Ay Veri ile)
"""
...
def retrain_model_1month():
    """GMSTR model güncelleme (son 1 ay veri ile)"""
    symbol = "GMSTR"
    
    client = BISTExchangeClient()
    
    logger.info(f"Model güncelleme: {symbol} (son 1 ay veri)")
    ...
    # 4h için model eğit
    logger.info("4h fiyat tahmin modeli güncelleniyor...")
    X_4h = df[features]
    y_4h = df['target_4h']
    ...
    logger.info("1gün fiyat tahmin modeli güncelleniyor...")
    X_1d = df[features]
    y_1d = df['target_1d']
    ...
    # Modelleri kaydet
    import joblib
    joblib.dump(model_4h, f"price_prediction_{symbol}_4h_updated.pkl")
    joblib.dump(model_1d, f"price_prediction_{symbol}_1d_updated.pkl")
    
    logger.info("=" * 60)
    logger.info("GÜNCELLENMİŞ MODELLER KAYDEDİLDİ")
    logger.info("=" * 60)
    logger.info(f"4h Model: price_prediction_{symbol}_4h_updated.pkl")
    logger.info(f"1gün Model: price_prediction_{symbol}_1d_updated.pkl")
    logger.info("=" * 60)
