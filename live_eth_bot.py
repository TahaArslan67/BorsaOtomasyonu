...
def live_eth_bot():
    ...
    logger.info("=" * 60)
    logger.info("ETH CANLI YÖN TAHMİN BOTU BAŞLADI")
    logger.info("=" * 60)
    logger.info("4h Walk-Forward: %4.21")
    logger.info("1gün Walk-Forward: %13.27")
    logger.info("=" * 60)
        ...
        try:
            ...
            try:
                ...
                if signal_4h_changed or signal_1d_changed or price_changed:
                    ...
                    if signal_4h == 1:
                        logger.info(f"4h: YÜKSELİŞ bekleniyor (Walk-Forward: %4.21) | Tahmin: ${pred_price_4h:.2f}")
                    else:
                        logger.info(f"4h: DÜŞÜŞ bekleniyor (Walk-Forward: %4.21) | Tahmin: ${pred_price_4h:.2f}")
                    
                    if signal_1d == 1:
                        logger.info(f"1gün: YÜKSELİŞ bekleniyor (Walk-Forward: %13.27) | Tahmin: ${pred_price_1d:.2f}")
                    else:
                        logger.info(f"1gün: DÜŞÜŞ bekleniyor (Walk-Forward: %13.27) | Tahmin: ${pred_price_1d:.2f}")
