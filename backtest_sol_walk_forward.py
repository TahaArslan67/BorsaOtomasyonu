...
def backtest_sol_walk_forward():
    """SOL walk-forward test"""
    symbol = "SOLUSDT"
    
    client = CryptoExchangeClient()
    
    logger.info(f"SOL walk-forward test: {symbol}")
    ...
    logger.info("=" * 60)
    logger.info("SOL WALK-FORWARD TEST")
    logger.info("=" * 60)
    
    # Modelleri yükle
    import joblib
    model_4h = joblib.load(f"price_prediction_SOL_4h.pkl")
    model_1d = joblib.load(f"price_prediction_SOL_1d.pkl")
