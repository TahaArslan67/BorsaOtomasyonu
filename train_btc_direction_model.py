...
def train_btc_direction_model():
    """BTC yön tahmin modeli eğit"""
    symbol = "BTCUSDT"
    
    client = CryptoExchangeClient()
    
    logger.info(f"BTC yön tahmin modeli eğitimi: {symbol}")
    ...
    # Modelleri kaydet
    import joblib
    joblib.dump(model_4h, f"direction_prediction_BTC_4h.pkl")
    joblib.dump(model_1d, f"direction_prediction_BTC_1d.pkl")
    
    logger.info("=" * 60)
    logger.info("BTC YÖN TAHMİN MODELLERİ KAYDEDİLDİ")
    logger.info("=" * 60)
    logger.info(f"4h Model: direction_prediction_BTC_4h.pkl")
    logger.info(f"1gün Model: direction_prediction_BTC_1d.pkl")
    logger.info("=" * 60)
