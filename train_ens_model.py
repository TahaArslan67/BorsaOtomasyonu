"""
ENS Model Eğitimi (Geliştirilmiş)
"""
...
def train_ens_model():
    """ENS model eğit (geliştirilmiş)"""
    symbol = "ENSUSDT"
    
    client = CryptoExchangeClient()
    
    logger.info(f"ENS model eğitimi (geliştirilmiş): {symbol}")
    ...
    # Modelleri kaydet
    import joblib
    joblib.dump(model_4h, f"price_prediction_ENS_4h_improved.pkl")
    joblib.dump(model_1d, f"price_prediction_ENS_1d_improved.pkl")
    
    logger.info("=" * 60)
    logger.info("ENS GELİŞTİRİLMİŞ FİYAT TAHMİN MODELLERİ KAYDEDİLDİ")
    logger.info("=" * 60)
    logger.info(f"4h Model: price_prediction_ENS_4h_improved.pkl")
    logger.info(f"1gün Model: price_prediction_ENS_1d_improved.pkl")
    logger.info("=" * 60)
