...
def backtest_btc_walk_forward():
    """BTC walk-forward test"""
    symbol = "BTCUSDT"
    
    client = CryptoExchangeClient()
    
    logger.info(f"BTC walk-forward test: {symbol}")
    ...
    # Walk-forward test
    # İlk 500 bar ile eğit, son 500 bar ile test
    train_size = 500
    test_size = 500
    ...
    # Model eğit (walk-forward)
    from sklearn.model_selection import train_test_split
    import xgboost as xgb
    
    # 4h model eğit
    X_train_4h = train_data[features]
    y_train_4h = train_data['close'].shift(-4)
    y_train_4h = y_train_4h.dropna()
    X_train_4h = X_train_4h.loc[y_train_4h.index]
    ...
    # 1gün model eğit
    X_train_1d = train_data[features]
    y_train_1d = train_data['close'].shift(-24)
    y_train_1d = y_train_1d.dropna()
    X_train_1d = X_train_1d.loc[y_train_1d.index]
    ...
    logger.info("=" * 60)
    logger.info("BTC WALK-FORWARD TEST")
    logger.info("=" * 60)
