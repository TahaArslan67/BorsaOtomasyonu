"""
GMSTR Canlı Yön Tahmin Botu
%91.48 (4h) ve %94.68 (1gün) doğrulukta
"""
...
def live_direction_bot():
    """GMSTR canlı yön tahmin botu"""
    symbol = "GMSTR"
    
    client = BISTExchangeClient()
    
    # Yön tahmin modellerini yükle (%91.48 4h, %94.68 1gün)
    import joblib
    ...
    logger.info("=" * 60)
    logger.info("GMSTR CANLI YÖN TAHMİN BOTU BAŞLADI")
    logger.info("=" * 60)
    logger.info("4h Doğruluk: %91.48")
    logger.info("1gün Doğruluk: %94.68")
    logger.info("=" * 60)
        ...
        try:
            ...
            try:
                ...
                if signal_4h_changed or signal_1d_changed or price_changed:
                    ...
                    if signal_4h == 1:
                        logger.info(f"4h: YÜKSELİŞ bekleniyor (Doğruluk: %91.48) | Tahmin: ₺{pred_price_4h:.2f}")
                    else:
                        logger.info(f"4h: DÜŞÜŞ bekleniyor (Doğruluk: %91.48) | Tahmin: ₺{pred_price_4h:.2f}")
                    
                    if signal_1d == 1:
                        logger.info(f"1gün: YÜKSELİŞ bekleniyor (Doğruluk: %94.68) | Tahmin: ₺{pred_price_1d:.2f}")
                    else:
                        logger.info(f"1gün: DÜŞÜŞ bekleniyor (Doğruluk: %94.68) | Tahmin: ₺{pred_price_1d:.2f}")
