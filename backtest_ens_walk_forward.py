...
def backtest_ens_walk_forward():
    """ENS walk-forward test"""
    symbol = "ENSUSDT"
    
    client = CryptoExchangeClient()
    
    logger.info(f"ENS walk-forward test: {symbol}")
    ...
    logger.info("=" * 60)
    logger.info("ENS WALK-FORWARD TEST")
    logger.info("=" * 60)
    
    # Geliştirilmiş modelleri yükle
    import joblib
    model_4h = joblib.load(f"price_prediction_ENS_4h_improved.pkl")
    model_1d = joblib.load(f"price_prediction_ENS_1d_improved.pkl")
